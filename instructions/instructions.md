...
Rule: Avoid using the `this` keyword when it can be avoided.
Reason: To prefer functional approaches that do not rely on `this` binding.

Rule: Avoid using classes when functional alternatives exist.

Rule: When a file operation fails because the file has not been read yet, read the file first before retrying the operation.
